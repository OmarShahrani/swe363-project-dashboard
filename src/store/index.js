...
const store = new Vuex.Store({
  ...
  getters: {
    ...
    inactive_services: state => state.inactive_services,
  ...
  mutations: {
    ...
    set_inactive_services: (state, payload) =>
      (state.inactive_services = payload)
  },
  actions: {
    INIT_APP: async ({ commit, dispatch }) => {
      ...
      try {
        ...
        const iadata = await usersdb.find({
          selector: { _id: "inactive_services" }
        });
        const iadocs = iadata.docs || [];
        const { inactive_services = [] } = iadocs[0];
        commit("set_inactive_services", inactive_services);
    ...
    TOGGLE_SERVICE: async ({ commit, getters }, name) => {
      let inactive_services = getters.inactive_services;
      const remotedb = await db();
      const data = await remotedb.find({
        selector: { _id: "inactive_services" }
      });
      const docs = data.docs || [];
      const { _id = null, _rev = null } = docs[0];
      const inactive = inactive_services.includes(name);
      if (inactive) {
        inactive_services = inactive_services.filter(v => v !== name);
      } else {
        inactive_services.push(name);
      }
      if (_id === null) {
        await remotedb.put({ _id: "inactive_services", inactive_services });
      } else {
        await remotedb.put({
          _id: "inactive_services",
          _rev,
          inactive_services
        });
      }
